...
$currdate = date("Y-m-d");
$data = $_POST;
$check_wfh_query = "SELECT * FROM punch_in_out WHERE wfh_status = 1 AND date = '$currdate' and punchedin_status = 1 ";
...
if (mysqli_num_rows($wfh_result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($row['1st_notification_status'] == 0) {
            ...
            //  $next_notification_time = date("H:i:s", strtotime($punch_in_time . ' +10 minutes'));
            ...
            // $next_notification_time = date("H:i:s", strtotime($row['1st_notification_time'] . ' +10 minutes'));
            ...
            //  $next_notification_time = date("H:i:s", strtotime($row['2nd_notification_time'] . ' +10 minutes'));
        } else {
            continue;
        }
