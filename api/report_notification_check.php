...
$query_approved = "SELECT * FROM report_notification WHERE is_reply = 0 and date(created_at) = '$todays_date' ";
...
if(mysqli_num_rows($result_approved)>0){
    ...
    // $getTokensQuery = mysqli_query($con, "SELECT fcm_token FROM user_login WHERE id = '3'");
    ...
    while ($tokenRow = mysqli_fetch_assoc($getTokensQuery)) {
        ...
        if (!empty($fcmToken)) {
            $responseFCM = sendNotification($fcmToken, "⏳ Report Notification Reply Reminder ", "Application Name: " . $application_name . " | Message: " . $purpose);
