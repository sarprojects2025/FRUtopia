...
$query_approved = "SELECT * FROM apply_work_from_home 
...
while ($row = mysqli_fetch_assoc($result_approved)) {
    ...
    if ($fcmToken) {
        $response = sendNotification($fcmToken, "📅 Work From Home Approved", "Your work from home request has been approved from ". $_fromdate ."to ".$_todate );

        $response_data = json_decode($response, true);
        if (isset($response_data['name'])) {
            // नोटिफिकेशन भेजने के बाद approval_notification_status को 1 में अपडेट करें
            $update_query = "UPDATE apply_work_from_home 
...
$query_rejected = "SELECT * FROM apply_work_from_home 
...
while ($row = mysqli_fetch_assoc($result_rejected)) {
    ...
    if ($fcmToken) {
        $response = sendNotification($fcmToken, "Work From Home Rejected", "Your work from home request has been rejected from ". $_fromdate ."to ".$_todate   );
